  ...
  var test = "" // empty string to store the boolean of the check
    for (var i = 0; i <= str.length; i++) { // for loop to loop through given string
        if (str[i] === "a") { // checks whether or not the character at [i] is "a"
            if (str[i-4] === "b" || str[i+4] === "b") { // if yes, checks to see if the letter three spaces away is "b" (for more information on selecting a character, go here https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/String/substr )
